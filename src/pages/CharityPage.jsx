...
function CharityPage() {
  ...
  const [visibleCount, setVisibleCount] = useState(10); // Hiển thị 10 chương trình đầu tiên
  ...
  useEffect(() => {
    const loadCharities = async () => {
      ...
      try {
        ...
        // Backend trả về tất cả data: { success: true, data: { charities: [...], pagination: {...} } }
        const charities =
        ...
        {isLoading ? (
          ...
          <>
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {memoizedCharityPrograms.slice(0, visibleCount).map((program) => (
                <div
                  key={program.id}
            ...
            {/* Nút Xem thêm */}
            {memoizedCharityPrograms.length > 10 &&
              visibleCount < memoizedCharityPrograms.length && (
                <div className="text-center pt-8">
                  <button
                    onClick={() => {
                      // Tăng thêm 10 chương trình để hiển thị
                      const nextVisible = Math.min(
                        visibleCount + 10,
                        memoizedCharityPrograms.length
                      );
                  ...
                  >
                    <span>
                      Xem thêm ({Math.max(
                        0,
                        memoizedCharityPrograms.length - visibleCount
